...
- `record_versions` — person별 정본 JSON, content_hash, tx_hash,
  상태(`draft → anchored`; 브라우저가 트랜잭션 컨펌까지 기다린 뒤 보고하므로
  중간 `pending` 상태는 두지 않고, 실패 시 앵커 페이지 재방문으로 재시도)
...
초기 데이터: 독립유공자 5명 인라인 시드(김구, 안중근, 유관순, 윤봉길, 안창호).
공훈전자사료관 공공데이터 대량 임포트는 후속 작업.
